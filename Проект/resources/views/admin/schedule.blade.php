...
@section('admin-content')
    ...
    <div class="admin-list">
        ...
        <form method="get" action="{{ route('admin.schedule') }}" style="display:flex; gap:12px; align-items:center; margin-bottom:12px;">
            ...
            <a href="{{ route('admin.schedule') }}" class="btn btn-outline">Сброс</a>
        </form>
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                @foreach($schedule as $item)
                    <tr>
                        ...
                        <td>
                            <button type="button" class="btn btn-outline js-edit-schedule"
                                data-id="{{ $item->id }}"
                                data-name="{{ $item->name }}"
                                data-date="{{ $item->date }}"
                            ...
                            <form action="{{ route('admin.delete-schedule', $item->id) }}" method="post" style="display:inline;">
                                @csrf
                                @method('DELETE')
    ...
    <div id="editScheduleModal" class="modal hidden">
        ...
        <div class="modal-panel" role="dialog" aria-modal="true" aria-labelledby="editScheduleTitle">
            <button class="close-btn" type="button" onclick="closeScheduleModal()" style="position:absolute; right:12px; top:12px;">
